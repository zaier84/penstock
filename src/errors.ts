...
/**
 * Base class for every error penstock throws. Sets a precise `name`,
...
/**
 * Synchronous misuse: bad construction, a missing `run`, duplicate
 * names, an unknown engine reference, or a reserved/unsafe name. Thrown
...
/**
 * Wraps a step `run` failure. Carries the failing step's name and
 * preserves the original error as `.cause`, used to populate `StepReport.error`
 * and `result.error` without leaking raw payloads.
 */
 ...
 * . Carries the full `Result`, the originating step failure as `.cause`,
